...
def getMean(arrred,numun,aiglabs,nsams):
	#################3arrred is rowwise################
	#print(arrred)
	#print(numun)
	#print(aiglabs)
	...
	#sys.exit()
	aiglabs = np.array(aiglabs)



	mean = np.zeros((numun,32))
	count = np.zeros((1,numun))
	i = 0
	while (i < nsams):
		count[0][aiglabs[i]] += 1
		j = 0
		while (j < 32):
			mean[aiglabs[i]][j] += arrred[i][j]
			j += 1
...
def getCovariance(mean,arrred,aiglabs,numun,nsams):

	aiglabs = np.array(aiglabs)
	covar = np.zeros((numun,32))
	count = np.zeros((1,numun))
	i = 0
	while (i < nsams):
		count[0][aiglabs[i]] += 1
		j = 0
		while (j < 32):
			covar[aiglabs[i]][j]  += (arrred[i][j] - mean[aiglabs[i]][j])**2
			j += 1
...
def classify3(newR,mean,covar,numun,nsams2,count,nsams):
	############newR is rowwise##########
	###first class:
	classified = []
	t = 0
	while (t < nsams2):
		...
		i = 0
		while (i < 32):
			fung += -0.5*((newR[t][i]-mean[0][i])**2)/covar[0][i]  + -0.5*math.log(2*(3.14159)*covar[0][i])	
			i += 1

		fung += math.log((count[0][0]/nsams))	
		...
		while (c < numun):
			...
			i = 0
			while (i<32):
				func += -0.5*((newR[t][i]-mean[c][i])**2)/covar[c][i] + -0.5*math.log(2*(3.14159)*covar[c][i])
				i += 1

			func += math.log((count[0][c]/nsams))
...
mean = getMean(arrred,numun,aiglabs,nsams)
covar, count = getCovariance(mean,arrred,aiglabs,numun,nsams)
...
classified = classify3(newR,mean,covar,numun,nsams2,count,nsams)
